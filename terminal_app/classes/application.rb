...
###############################################  PICK ITEM IN MAP ############################################
    ...
    def find_my_object() # (dot_array, dot_array_presentations)
        ...
        @updated_good_causes.each do |object| #iterate through objects, and see if their presentation matches the picked one
                if object.presentation.chomp == user_choice
...
###############################################  VIEW MAP AND SEE CAUSES  ############################################  
        ...
        if menu_selection == @main_menu.menu_options[0]
            ...
            choices = ["1. Support the cause for #{selected_cause.cost} Charity Coins", "2. Find another cause to support", "3. Return to Main Menu"] 
...
###############################################  INCREASE CHARITY CHEST ############################################
            ...
            more_coins_options = ["Would you like to spend money from your account ?", "Would you like to earn some more money ?", "How about asking for a group contribution?"]
...
###############################################  SEE PROGRESS  ############################################            
            ...
            puts "Sorry, no progress just yet"
            @main_menu.return_to_main
